...
RenderTemplateForNode and render_template_for are riped directly from
Christian Mett's mint_django_utils. Moar better licensing ftw!

    https://github.com/mintchaos/mint_django_utils/blob/
...
def get_latest_content(parser, token):
    ...
    if bits[3] != 'as':
        raise template.TemplateSyntaxError("third argument to get_latest_content tag must be 'as'")
    return ContentNode(bits[1], bits[2], bits[4])
